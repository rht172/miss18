...
class czDBAccess
{
    ...
    public function selectDataGroupby($tempSelectFeilds, $tempWhereStatement, $limit = "")
    {
        $db = Database::getInstance();
        $mysqli = $db->getConnection();
        $myArray = array();
        if (strlen($tempWhereStatement) > 0) {
            $sql = "SELECT $tempSelectFeilds FROM $this->varTableName where $tempWhereStatement $limit GROUP BY device_code;";

        } else {
            $sql = "SELECT $tempSelectFeilds FROM $this->varTableName $limit GROUP BY device_code;";
        }
    ...
    public function selectDataGroupbyAll($tempSelectFeilds, $tempWhereStatement, $limit = "", $groupBy)
    {
        $db = Database::getInstance();
        $mysqli = $db->getConnection();
        $myArray = array();
        if (strlen($tempWhereStatement) > 0) {
            $sql = "SELECT $tempSelectFeilds, sum(qty) as tot_qty FROM $this->varTableName where $tempWhereStatement GROUP BY $groupBy order by tid desc $limit ;";

        } else {
            $sql = "SELECT $tempSelectFeilds, sum(qty) as tot_qty FROM $this->varTableName GROUP BY $groupBy order by tid desc $limit;";
        }
    ...
    public function selectDataByOrder($tempSelectFeilds, $tempWhereStatement, $limit = "")
    {
        $db = Database::getInstance();
        $mysqli = $db->getConnection();
        $myArray = array();
    ...
    public function selectDataOrderByEidLike($tempSelectFeilds, $tempWhereStatement, $limit = "")
    {
        $db = Database::getInstance();
        $mysqli = $db->getConnection();
        $myArray = array();
        if (strlen($tempWhereStatement) > 0) {
            $sql = "SELECT $tempSelectFeilds FROM $this->varTableName where $tempWhereStatement order by employee_id DESC $limit;";

        } else {
            $sql = "SELECT $tempSelectFeilds FROM $this->varTableName order by employee_id DESC $limit;";
        }
    ...
    public function selectDataForTwoTables($tempFirstTable, $tempSecondTable, $tempWhereStatement)
    {
        $db = Database::getInstance();
        $mysqli = $db->getConnection();
        $myArray = array();
        if (strlen($tempWhereStatement) > 0) {
            $sql = "SELECT $tempFirstTable.device_code,COUNT($tempFirstTable.tid) AS NumberOfDays, $tempFirstTable.*, $tempSecondTable.* FROM $tempFirstTable LEFT JOIN $tempSecondTable ON $tempFirstTable.device_code = $tempSecondTable.device_no where $tempWhereStatement GROUP BY $tempFirstTable.device_code;";

        } else {
            $sql = "SELECT $tempFirstTable.device_code,COUNT($tempFirstTable.tid) AS NumberOfDays, $tempFirstTable.*, $tempSecondTable.* FROM $tempFirstTable LEFT JOIN $tempSecondTable ON $tempFirstTable.device_code = $tempSecondTable.device_no GROUP BY $tempFirstTable.device_code;";
        }
    ...
    public function selectDataForTwoTablesLogLedger($tempFirstTable, $tempSecondTable, $tempWhereStatement)
    {
        $db = Database::getInstance();
        $mysqli = $db->getConnection();
        $myArray = array();
        if (strlen($tempWhereStatement) > 0) {
            $sql = "SELECT $tempFirstTable.device_code, $tempFirstTable.*, $tempSecondTable.* FROM $tempFirstTable INNER JOIN $tempSecondTable ON $tempFirstTable.device_code = $tempSecondTable.device_no where $tempWhereStatement GROUP BY $tempFirstTable.device_code;";

        } else {
            $sql = "SELECT $tempFirstTable.device_code, $tempFirstTable.*, $tempSecondTable.* FROM $tempFirstTable INNER JOIN $tempSecondTable ON $tempFirstTable.device_code = $tempSecondTable.device_no GROUP BY $tempFirstTable.device_code;";
        }
    ...
    public function selectDataForThreeTables($tempFirstTable, $tempSecondTable, $tempThirdTable, $tempWhereStatement)
    {
        $db = Database::getInstance();
        $mysqli = $db->getConnection();
        $myArray = array();
        if (strlen($tempWhereStatement) > 0) {
            $sql = "SELECT $tempFirstTable.*, $tempSecondTable.* FROM $tempFirstTable LEFT JOIN $tempSecondTable ON $tempFirstTable.device_code = $tempSecondTable.device_no JOIN $tempThirdTable ON $tempFirstTable.device_code = $tempThirdTable.device_no where $tempFirstTable.$tempWhereStatement;";

        } else {
            $sql = "SELECT $tempFirstTable.*, $tempSecondTable.* FROM $tempFirstTable LEFT JOIN $tempSecondTable ON $tempFirstTable.device_code = $tempSecondTable.device_no;";
        }
    ...
    // public function selectDataForAutoCompleteLike($tempquery, $feildName)
    // {
    //     $db = Database::getInstance();
    //     $mysqli = $db->getConnection();
    //     $sql = $tempquery;
