  ...
  if (storedJsonString != null) {
    ...
    function processArray(index) {
      if (index < storedArray.length) {
        ...
        xhr_8.onreadystatechange = function() {
          if (xhr_8.readyState === 4 && xhr_8.status === 200) {
            ...
            <div class="pt-2 pt-sm-0 ps-sm-3 mx-auto mx-sm-0 text-center text-sm-end" style="max-width: 9rem;">
              <p class="mb-0"><span class="text-muted fs-sm">Quantity:</span><span>&nbsp;<span class='quantity'>${qty}</span>${subscribe > 0 ? `<span class='quantity'>&nbsp;&times;&nbsp;${subscribe}&nbsp;Month</span>` : ''}</span></p>
            </div>
        ...
        // Example: Trigger a function when all iterations are complete
        allIterationsCompleted();
      }
    }

    // Start the loop with the first call
    processArray(0);

    // Function to execute when all iterations are complete
    function allIterationsCompleted() {
          ...
          if (insertId == "failure") {
            ...
            if (insertId > 0) {
              ...
              if (storedJsonString != null) {
                ...
                windows.location.href = "checkout-complete.php?shd=" + insertId + "&g_t_v=" + g_t_v + "&reward_ppp=" + reward_points_1 + "&dshjbhsdghsdf=Success";
