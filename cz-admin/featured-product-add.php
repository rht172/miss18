...
<div class="container-fluid">
    <h1>Featured product Master</h1><br>


    <form action="featured-product-ctrl.php?key1=<?php echo $processName; ?>" method="post"
        ...
        <div class="form-row">
            <div class="form-group   col-md-3">
                ...
                <select class="form-control" id="sku" name="sku" autofocus required>
                    <option value=""> </option>
                    <?php
                    $result = $obj_class_product->selectData_customqry("select distinct sku from product_table_ecom");
                    while ($row = $result->fetch_assoc()) {
                        if ($sku == $row['sku']) {
                            echo ' <option selected="selected" value="' . $row['sku'] . '" >' . $row['sku'] . '</option>';
                        } else {
